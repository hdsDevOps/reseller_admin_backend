...
class NotificationController {
  ...
  async updateTemplate(req, res) {
    try {
      ...
      const result = await NotificationService.updateTemplate(record_id, template_content,is_notification);
      res.status(200).json(result);
    } catch (error) {
      res.status(400).json({ status: 'error', message: error.message });
    }
  ...
  async deleteTemplate(req, res) {
    try {
      const { record_id } = req.body;
      const result = await NotificationService.deleteTemplate(record_id);
      res.status(200).json(result);
    } catch (error) {
      res.status(400).json({ status: 'error', message: error.message });
    }
  ...
  async updatenotification_status(req, res){
    try {
      const data = req.body;
      const notifications = await NotificationService.getnotificationdetails(data);
      res.status(200).json(notifications);
  ...
  async readnotification(req, res){
    try {
      const data = req.body;
      const notifications = await NotificationService.readnotification(data);
      res.status(200).json(notifications);
    } catch (error) {
      res.status(400).json({ status: 'error', message: error.message });
    }
